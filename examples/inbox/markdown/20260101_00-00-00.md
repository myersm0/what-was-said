# source: JuliaNeuroscience/CIFTI.jl: A Julia package for reading CIFTI files for surface-space neuroimaging data - Brave
# CIFTI
...
### Alternative read methods for performance-critical situations

CIFTI.jl now provides two additional, experimental methods for reading data:
```julia
# just load the raw data matrix without any metadata or transposition:
my_matrix = CIFTI.load(CIFTI.MatrixOnly(), filename)

# memory-mapped loading for data that may exceed available RAM:
my_memory_mapped_matrix = CIFTI.load(CIFTI.MmapOnly(), "huge_dconn.nii")
```

It's important to understand that in either case above, you're accessing the data matrix exactly as it's stored on disk, without any transposition. It likely will not be in the probably-familiar grayordinates x timepoints orientation.

For additional lazy loading strategies like partial file access (e.g., reading specific columns), it's fairly easy to do so yourself with the tools provided by this package. See the demo file `examples/lazy.jl` in this repo for a demonstration.
