...
**Near-duplicate detection**: After parsing but before inserting, computes a document-level MinHash signature and compares against existing documents within a ±180 day window (`dup_window_days`), ordered by `(clip_date, id)`. The decision combines two signals: Jaccard similarity over the MinHash signatures, and `minhash::longest_shared_block_words` (the longest contiguous run of shared shingles, in words). A candidate is the *same version* when Jaccard ≥ `dup_jaccard_high` (0.7), or when Jaccard ≥ `dup_jaccard_low` (0.4) and the shared block ≥ `dup_shared_block_words` (300). Jaccard between `dup_jaccard_low` and `dup_jaccard_high` with a smaller block is the gray zone, resolved interactively. Below `dup_jaccard_low` is not a match.

The candidate loop does **not** break early: a `superseded` relation is written for *every* same-version candidate, then `recompute_supersession` recomputes the affected family. This separates two questions the old loop conflated — which documents are one version family (similarity) and which is current (time). The shared block is still skipped (recorded NULL) for a candidate cleared on Jaccard alone, since Jaccard runs on the stored signatures whereas the block needs a text fetch plus an O(m·n) LCS.
...
12. For each candidate, evaluate Jaccard and (in the gray band) longest shared block:
    a. Jaccard ≥ 0.7, or Jaccard ≥ 0.4 with shared block ≥ 300 words → same-version candidate (plan a `superseded` edge; loop does not break)
    b. 0.4 ≤ Jaccard < 0.7 with smaller block → gray zone; prompt about the chronological predecessor (no controlling TTY → abort this file)
