...
Subcommands: ingest (file or directory), about (with `--method=[exact|semantic]`), in, dump, stats, embed, derive, extract, diff, relations repair (with `--family`), relations scan, serve, sync, browse (default).
...
**Scan** (`scan_relations`, `what-was-said relations scan`): retroactive near-duplicate detection over the whole collection, for pairs that ingest-time detection missed (docs ingested before the detector or a given signal existed, pairs outside the ±180-day window, or estimator draws below a band). Because it is offline, it uses **exact** text similarity — per-document shingle sets computed once, exact Jaccard and containment from set intersections — so estimator noise cannot cause a miss; the MinHash signature only serves the ingest fast path. Candidates are captured docs only (`project IS NULL`, non-positional). Pairs with any existing relation (`superseded` *or* `kept_both` — a recorded human judgment is never re-asked) are skipped, as are pairs already in one family transitively (a union-find over `superseded` edges, updated as the scan links, so healing a three-doc family takes exactly two decisions). `classify_exact` mirrors `classify_by_signature`'s routes on exact values: Jaccard ≥ 0.7 auto-links (and recomputes the family, same as ingest); the gray band, the title route, containment ≥ 0.8 over the size floor, and the ≥ 300-word block each route to the same `prompt_gray_zone`, iterating nearest-predecessor first. Every decision is written in its own transaction with an immediate `recompute_supersession`, so `[q]uit` keeps all progress. With no controlling TTY, the scan aborts at the first pair needing judgment, naming it.
...
- `estimated_overlap()`: Overlap coefficient (|A∩B| / min(|A|,|B|)) recovered algebraically from a Jaccard estimate and the two set cardinalities, with no text access. A cheap signature-only pre-filter for the containment path; an estimate over k=128 hashes, so it only gates the text fetch rather than deciding
