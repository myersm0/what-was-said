...
**Near-duplicate detection**: After parsing but before inserting, computes a document-level MinHash signature and compares against existing documents within a ±180 day window (`dup_window_days`), ordered by `(clip_date, id)`. Each candidate is classified by `classify_by_signature` — a pure function of the Jaccard estimate and the two documents' shingle-count cardinalities, with no text fetch or IO — which returns one of four verdicts:
...
11. **Near-duplicate detection, and what may merge unattended**: Documents are fingerprinted at ingest using 3-word shingle MinHash signatures and compared against existing docs within a configurable window (default ±180 days). Classification (`classify_by_signature`) draws on three signals: Jaccard similarity, a longest-shared-block heuristic, and a containment (overlap-coefficient) signal. Only a **high Jaccard** (≥ 0.7) merges unattended into a version family; the block and containment signals only ever route a candidate to the interactive prompt. This asymmetry is deliberate and hard-won: "large shared block" and "one body contained in the other" are each equally the fingerprint of a genuine revision *and* of two distinct documents built on shared boilerplate, a template, or a skeleton. Letting either auto-merge lets one boilerplate-heavy newcomer weld unrelated documents into one family and hide all but the latest — a silent over-*hide* that is worse than the redundant over-*show* it would replace. So tolerance-grade evidence is confirmed by a human; only equivalence-grade Jaccard is trusted to merge on its own. Same-version candidates are linked into a version family; the family's `superseded` tags are then recomputed so the latest by `(clip_date, id)` is current and the rest are tagged `superseded` (both preserved). The block and containment are not computed for a Jaccard-high candidate (they would cost a text fetch plus an O(m·n) LCS the decision doesn't need), so `shared_block_words` is NULL in that case.
...
21. **Cheap signatures gate expensive text**: Both text-based signals (longest-shared-block and exact containment) cost a text fetch plus an O(m·n) pass, so they are gated behind cheap signature-only checks that run on the stored MinHash and shingle counts. The block is computed only once a candidate is already in the moderate Jaccard band; exact containment only once a sub-0.4 candidate clears the size floor and the algebraic `estimated_overlap` pre-filter. The block itself is computed over interned 3-word shingle ids (a rolling-DP longest-common-substring), not characters — the question is only "is there a big shared block?", so shingle granularity suffices, reuses the existing tokenizer, and is reported as an approximate word count. Neither text signal authorizes an automatic merge; both only inform the interactive prompt (see decision 11).
